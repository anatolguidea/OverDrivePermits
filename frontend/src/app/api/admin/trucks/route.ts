...
import { findTrucksByCustomer, createTruck } from '@/lib/repositories/trucks.repo'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await findTrucksByCustomer(supabase, customer_id)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const parsed = parseWithSchema(truckSchema, rest)
    const normalized = normalizeTruck(parsed)
    const data = await createTruck(supabase, {
      ...normalized,
      customer_id: scoped.customer_id,
    } as Parameters<typeof createTruck>[1])
